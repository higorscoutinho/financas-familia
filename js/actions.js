/* =========================================================
   actions.js
   MUDANÇAS: saveConfig agora salva dataInicioSistema,
   saveNota e removeNota adicionados
   ========================================================= */
...
const Actions = {
  ...
  payFixedBill(id) {
    ...
    if (divVinc) {
      if (novoPago) {
        Store.update("dividas","Dividas", divVinc.id, { status:"quitada", valorPago: divVinc.valorOriginal, valorRestante: 0 });
        ...
        Store.update("dividas","Dividas", divVinc.id, { status:"em_atraso", valorPago: 0, valorRestante: divVinc.valorOriginal });
  ...
  registrarPagamentoDivida(dividaId, valor, data, obs) {
    ...
    if (!divida) return;
    const pag = { id: Utils.uid("pag"), dividaId, valor: Number(valor)||0, data: data||Utils.todayISO(), obs: obs||"", criadoPor: App.currentUser };
    Store.data.pagamentosDividas.push(pag);
    Store.saveLocal();
    Store.queueChange("PagamentosDividas","create", pag);
    Store.addHistoricoDivida(dividaId,"pagamento","Pagamento registrado", pag.valor);
    Store.recalcularDivida(dividaId);
    Utils.toast(`Pagamento de ${Utils.brl(pag.valor)} registrado ✓`);
    Pages.render(App.currentPage);
  },
  ...
  negociarDivida(dividaId, dados) {
    ...
    Store.data.negociacoesDividas.push(neg);
    Store.update("dividas","Dividas", dividaId, {
      valorAtual:    neg.valorNegociado,
      valorRestante: Math.max(0, neg.valorNegociado - Number(divida.valorPago||0)),
      status:        neg.numeroParcelas > 1 ? "parcelada" : "negociada",
    });
    Store.saveLocal();
    Store.queueChange("NegociacoesDividas","create", neg);
    Store.addHistoricoDivida(dividaId,"negociacao",`Negociada: ${Utils.brl(neg.valorOriginal)} → ${Utils.brl(neg.valorNegociado)} (economia ${Utils.brl(neg.economia)})`, neg.valorNegociado);
    Utils.toast(`Economia de ${Utils.brl(neg.economia)} ✓`);
    Pages.render(App.currentPage);
  },
  ...
  quitarDivida(id) {
    ...
    Store.update("dividas","Dividas", id, { status:"quitada", valorPago: divida.valorAtual, valorRestante: 0 });
    ...
    if (divida.contaFixaId) {
      ...
      if (conta && !conta.pago) Store.update("contasFixas","ContasFixas", conta.id, { pago:true, dataPagamento: Utils.todayISO(), horaPagamento: new Date().toLocaleTimeString("pt-BR") });
    }
  ...
  saveConfig() {
    ...
    Store.saveLocal();
    Store.queueChange("Configuracoes","update",{ metaEconomiaMensal: meta, diasToleranciaAtraso: tol, dataInicioSistema: inicio });
