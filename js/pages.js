const Pages={
  ...
  dashboard(main){
    ...
    // Contas do mês inclui aportes de investimento como custo
  ...
  parcelamentosPage(main){
    ...
    card.innerHTML=this.listOrEmpty(Store.data.parcelamentos,p=>{
      ...
      if(ativo){
        if(pago){badge=`<span class="badge green">Pago</span>`;}
        else{
  ...
  // ═══ INVESTIMENTOS ══════════════════════════════════════════
  // Investimentos são separados de Contas Fixas.
  // O aporte mensal entra como custo no dashboard (totalInv),
  // mas NÃO gera fixa automática.
  // Aqui: apenas consulta e registro de aportes avulsos.
  investimentos(main){
    ...
    if(invs.length){
      ...
      r.innerHTML=`
        ...
        ${totalAportes>0?this.statCard("amber","💰","Aportes mensais",Utils.brl(totalAportes),"Incluído nas Contas do mês"):""}
      `;
      main.appendChild(r);
    }
    ...
    invs.forEach(inv=>{
      ...
      card.innerHTML=`
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4px;">
          ...
          ${aporte>0?`<span class="badge amber">+${Utils.brl(aporte)}/mês</span>`:""}
        </div>
  ...
  debtCard(d){
    ...
    return`<div class="debt-card debt-${d.status}">
      <div class="debt-card-header">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;"><span class="badge ${sCls}">${sLabel}</span><span class="badge ${pCls}">${pLabel}</span></div>
      </div>
